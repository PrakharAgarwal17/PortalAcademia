...
const OnboardingOtpMap = new Map();
...
function searchAishe(query, limit = 25) {
    if (!query || !aisheDataSet.length)
        return [];
    ...
    for (const inst of aisheDataSet) {
        ...
        const matchesPrefix = queryVariations.some((qv) => rawName.startsWith(qv) || normName.startsWith(qv) || code === qv || code.startsWith(qv));
        if (matchesPrefix) {
    ...
    for (const inst of aisheDataSet) {
        ...
        const isMatch = queryVariations.some((qv) => {
            if (rawName.includes(qv) || normName.includes(qv) || code.includes(qv))
                return true;
...
export async function searchInstitutions(req, res) {
    try {
        ...
        const filtered = query
            ? DEFAULT_INSTITUTIONS.filter((inst) => inst.name.toLowerCase().includes(lowerQuery) ||
...
export function deriveOfficialInstitutionEmails(institutionName, website) {
    if (website) {
        try {
...
// ============================================================
export async function crawlCollegeEmails(req, res) {
    try {
...
export async function verifyOnboardingOtp(req, res) {
    try {
