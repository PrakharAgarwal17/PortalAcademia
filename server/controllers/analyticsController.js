...
export async function getCohortAnalytics(req, res) {
    try {
        ...
        const studentMatch = { accountType: "student" };
        if (instName) {
            studentMatch.$or = [
        ...
        const curriculumDeficits = marketSkills.map((ms) => {
            ...
            const deficiencyPct = Math.max(5, Math.round(((totalStudents - studentCount) / totalStudents) * 100));
            return {
        ...
        const avgReadiness = Math.min(96, Math.max(45, Math.round((summary.avgSkills || 3) * 14 + verificationRate * 0.3)));
 ...
 */
export async function getStudentSkillGap(req, res) {
    try {
        if (!req.userId) {
            return res.status(401).json({ success: false, message: "Unauthorized" });
        }
