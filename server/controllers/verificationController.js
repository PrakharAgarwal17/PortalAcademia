...
export async function getPendingVerifications(req, res) {
    try {
        ...
        const studentFilter = { accountType: "student" };
        if (instName) {
            studentFilter.$or = [
                ...
                for (const cert of student.certifications) {
                    if (!cert.isVerified) {
                        pendingQueue.push({
                            ...
                            credentialId: cert._id?.toString() || "",
                            type: "certification",
                            title: cert.title,
                            issuer: cert.issuer || "",
...
export async function verifyCredential(req, res) {
    try {
        ...
        // Check certifications
        if (student.certifications && Array.isArray(student.certifications)) {
            const cert = student.certifications.id(credentialId);
            if (cert) {
                cert.isVerified = Boolean(isVerified);
                cert.verifiedBy = req.userId;
        ...
        if (!updatedItem && student.pastExperience && Array.isArray(student.pastExperience)) {
            const exp = student.pastExperience.id(credentialId);
            if (exp) {
                exp.isVerified = Boolean(isVerified);
                exp.verifiedBy = req.userId;
